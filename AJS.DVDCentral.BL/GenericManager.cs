...
using AJS.DVDCentral.PL2.Entities;
    ...
    public abstract class GenericManager<T> where T : class , IEntity
    {
        ...
        public async Task<List<T>> LoadAsync()
        {
            try
            {
                ...
                return await new DVDCentralEntities(options)
                    .Set<T>()
                    .ToListAsync<T>()
            ...
            bool rollback = false)
        {
            try
            {
                ...
                using (DVDCentralEntities dc = new DVDCentralEntities(options))
                {
                    ...
                    {
                        if(logger != null) { logger.LogWarning("Row already exists mate paws off"); }
                        throw new Exception("Row already exists mate");
                    }
